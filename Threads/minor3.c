 ...
 * 11/1/2021
 * CSCE 3600.002
 * This program uses condition variables to ensure nothing is added when the shared buffer between 2 threads
 * is full and nothing is echoed to the screen when the shared buffer between 2 threads is empty.
...
// (Condition Variables)
pthread_cond_t Full;	// Condition variable to handle when the shared buffer is full
pthread_cond_t Empty;	// Condition variable to handle when the shared buffer is empty
...
{ 
	...
	pthread_cond_init(&Full, NULL);		// Initialize conditionFull as NULL
	pthread_cond_init(&Empty, NULL);	// Initialize conditionEmpty as NULL
	...
	pthread_cond_destroy(&Full);	// Destroy conditionFull 
	pthread_cond_destroy(&Empty);	// Destroy conditionEmpty 
...
void * producer(void *arg)
{
	...
	while (1)
	{
		...
		// -- if not full, keep locked and proceed to place character on shared
		while (!(shared_count < NITEMS))
		{
			// If buffer is full, release mutex lock and sleep until signaled.
		...
		// Signal condition variable for buffer is not empty.
...
void * consumer(void *arg)
{
	...
	while (1)
	{
		...
		{
			// if buffer is empty, release mutex lock and sleep until signaled.
		...
		// Signal condition variable for buffer is not full.
